...
public class SocketServer5 {
	...
	private void start(int port) {
		...
		try (ServerSocket serverSocket = new ServerSocket(port);) {
			...
			rooms.add(lobby);
			while (SocketServer5.isRunning) {
				try {
					Socket client = serverSocket.accept();
					System.out.println("Client connecting...");
					// Server thread is the server's representation of the client
					ServerThread5 thread = new ServerThread5(client, lobby);
					thread.start();
	...
	protected void joinLobby(ServerThread5 client) {
		...
		if (joinRoom(LOBBY, client)) {
			...
			System.out.println("Added " + client.getClientName() + " to Lobby; Prelobby should self destruct");
		} else {
			System.out.println("Problem moving " + client.getClientName() + " to lobby");
		}
	}
	 ...
	 */
	protected synchronized boolean joinRoom(String roomName, ServerThread5 client) {
		...
		if (newRoom != null) {
			if (oldRoom != null) {
				System.out.println(client.getClientName() + " leaving room " + oldRoom.getName());
				oldRoom.removeClient(client);
			}
			System.out.println(client.getClientName() + " joining room " + newRoom.getName());
	...
	public static void main(String[] args) {
		...
		if (port > -1) {
			...
			SocketServer5 server = new SocketServer5();
